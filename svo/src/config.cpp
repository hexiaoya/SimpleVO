...
Config::Config() :
    ...
    grid_size(vk::getParam<int>("svo/grid_size", 30)), // 原程序30
    ...
    max_fts(vk::getParam<int>("svo/max_fts", 120)),// 原程序120
    quality_min_fts(vk::getParam<int>("svo/quality_min_fts", 50)),
    quality_max_drop_fts(vk::getParam<int>("svo/quality_max_drop_fts", 40)),
    quality_max_outliers(vk::getParam<int>("svo/quality_max_outliers", 40)) //知行合一 blog 更改条件1
#else
    trace_name("svo"),
    trace_dir("/tmp"),
    n_pyr_levels(3),
    ...
    core_n_kfs(3),
    map_scale(0.5),
    grid_size(10), // 修改重定位的第二个条件：修改grid大小
    ...
    max_n_kfs(0),
    img_imu_delay(0.0),
    max_fts(150),// 这个参数限制当投影的cell数目到了max_fts就不在投影
    quality_min_fts(50),
    quality_max_drop_fts(40),
    quality_max_outliers(40) //知行合一 blog 更改条件1
#endif
{}
