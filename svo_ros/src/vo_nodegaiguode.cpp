...
IplImage* image1=cvCreateImage(cvSize(640,480),8,1);
cv::Mat tmp_frame;
namespace svo {
...
public:
  ...
  bool publish_markers_;                 //!<只发步最少量的 (choice for embedded devices)
...
//==================================================
	...
	{
	*image = (IplImage)img;
	cvRemap(image,image1,mapx,mapy);
	 tmp_frame= cv::Mat(image1);
	}
//==================================================

  processUserActions();
  vo_->addImage(tmp_frame, msg->header.stamp.toSec());
  visualizer_.publishMinimal(tmp_frame, vo_->lastFrame(), *vo_, msg->header.stamp.toSec());
//============================输出时间和轨迹信息============================
  //double time_svo = vo_->lastProcessingTime();
  //svo::Frame* ff =vo_->lastFrame().get();
  //Sophus::SE3 T_R = ff->T_f_w_;
  // Eigen::Vector3d T_= T_R.translation();
  // std::cout<<time_svo<<" "<<T_(0,0)<<" "<<T_(1,0)<<" "<<T_(2,0)<<std::endl;
 // std::cout<<time_svo<<std::endl;
//============================输出时间和轨迹信息============================
...
int main(int argc, char **argv)
{
  ...
  svo::VoNode vo_node;
//==================================================
	CvMat* intrinsic=(CvMat*)cvLoad("/media/hexiaoya/新加卷/12335444/Intrinsics.xml");
	CvMat* distortion=(CvMat*)cvLoad("/media/hexiaoya/新加卷/12335444/Distortion.xml");
	cvInitUndistortMap(intrinsic,distortion,mapx,mapy);
//==================================================
  // subscribe to cam msgs
  //std::string cam_topic(vk::getParam<std::string>("svo/cam_topic", "camera/image_raw"));
  std::string cam_topic(vk::getParam<std::string>("svo/cam_topic", "/iarc007/mono_video"/*"/guidance/left_image"*/));
