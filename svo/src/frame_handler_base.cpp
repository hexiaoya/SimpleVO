...
void FrameHandlerBase::setTrackingQuality(const size_t num_observations,const size_t num_outliers)
{
  ...
  // 知行合一 blog 更改条件1
  /// 如果这次优化过程中的点比上一帧少很多，那也认为跟踪失败
  if(num_outliers > Config::qualityMaxOutliers())
  {
    SVO_WARN_STREAM("Lost "<< num_outliers <<" features!");
    tracking_quality_ = TRACKING_INSUFFICIENT;
  }
