...
public:
  ...
  /// outliers的数量不多于一个阈值. Tracking quality is bad.
  static size_t& qualityMaxOutliers() { return getInstance().quality_max_outliers; }//知行合一 blog 更改条件1
private:
  Config();
  Config(Config const&);
  ...
  size_t quality_max_outliers;//知行合一 blog 更改条件1
};
